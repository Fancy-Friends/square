...
npm run provider -- square            # regenerate this repo
npm run provider -- square --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/square-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/square-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/square-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-square` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/square/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/square` |
| npm | `@particle-academy/square-ui`, `@particle-academy/square-js` |
| Packagist | `particle-academy/square-php` |
| PyPI | `fancy-square` |
...
**Default to plain HTTP.** A Square SDK is third-party code and is subject to
